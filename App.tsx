  ...
  return (
    ...
    >
      <div className="w-5 h-4 flex flex-col justify-between">
        <span 
          className={`w-full h-0.5 transition-all duration-300 ${isOpen ? 'rotate-45 translate-y-1.5' : ''}`}
          style={{ backgroundColor: isDark ? '#ffffff' : '#030213' }}
        />
        <span 
          className={`w-full h-0.5 transition-all duration-300 ${isOpen ? 'opacity-0' : ''}`}
          style={{ backgroundColor: isDark ? '#ffffff' : '#030213' }}
        />
        <span 
          className={`w-full h-0.5 transition-all duration-300 ${isOpen ? '-rotate-45 -translate-y-1.5' : ''}`}
...
function Navigation({ activeLink, setActiveLink, isDark, toggleTheme, isMobile = false, onItemClick }: { 
  ...
  const handleItemClick = (item: string) => {
    if (item === 'Resume') {
      window.open('https://drive.google.com/file/d/1_CSHrYyRZnBfKeX2so6yJlHO0k5ji1Fw/view?usp=sharing', '_blank');
      ...
      <div className="flex flex-col gap-6 items-center justify-center p-8">
        {navItems.map((item) => (
          <button
            ...
            className={`font-medium leading-none relative shrink-0 text-lg text-center cursor-pixel transition-all duration-300 hover:scale-105`}
            style={{ 
              color: activeLink === item && item !== 'Resume' 
                ? (isDark ? '#ffffff' : '#030213')
                : (isDark ? '#b0b0b0' : '#717182')
            }}
          >
            <p className={`block leading-normal whitespace-pre transition-all duration-300 ${
              activeLink === item && item !== 'Resume'
                ...
                : 'hover:underline decoration-solid underline-offset-2'
            }`}>
              {item}
            </p>
          </button>
    ...
    <div className="hidden lg:flex flex-row gap-6 xl:gap-10 items-center justify-start p-0 relative shrink-0">
      {navItems.map((item) => (
        <button
          key={item}
          onClick={() => handleItemClick(item)}
          className={`font-medium leading-none relative shrink-0 text-sm text-left text-nowrap cursor-pixel transition-all duration-300 hover:scale-105 hover:-translate-y-0.5`}
          style={{ 
            color: activeLink === item && item !== 'Resume' 
              ? (isDark ? '#ffffff' : '#030213')
              : (isDark ? '#b0b0b0' : '#717182')
  ...
  return (
    <>
      ...
      <div 
        ...
        style={{
          backgroundColor: isScrolled ? (isDark ? 'rgba(18, 18, 18, 0.8)' : 'rgba(255, 255, 255, 0.8)') : 'transparent',
  ...
  return (
    <div className="absolute inset-0 flex items-center justify-center w-full h-full">
      <div className="w-full max-w-[1354px] h-full">
        ...
        >
          <g id="Grid Background" opacity={isDark ? "0.3" : "0.2"}>
            <path
              d="M1.00002 9L1 493"
...
function AvailabilityBadge({ isDark }: { isDark: boolean }) {
  return (
    <div 
      className={`${isDark ? 'bg-yellow-900/30 border border-yellow-700/50' : 'bg-yellow-100'} box-border content-stretch flex flex-row gap-2.5 items-start justify-center px-3 py-2 relative rounded-[100px] shrink-0 transition-all duration-300 hover:scale-105 cursor-pixel`}
    >
      <div className={`font-medium leading-none relative shrink-0 ${isDark ? 'text-yellow-400' : 'text-yellow-800'} text-xs sm:text-sm text-center text-nowrap transition-colors duration-300`}>
...
function HeroContent({ isDark }: { isDark: boolean }) {
  return (
    <div className="box-border content-stretch flex flex-col gap-4 items-center justify-start leading-none p-0 relative shrink-0 text-center">
      <div 
        className="font-bold relative shrink-0 text-2xl sm:text-3xl lg:text-[34px] text-center transition-colors duration-300"
        style={{ color: isDark ? '#ffffff' : '#030213' }}
      >
        <p className="block leading-normal whitespace-pre">
          I'm Kamran✌🏼
          <br />
      ...
      <div className="relative shrink-0 w-full max-w-4xl px-4 transition-colors duration-300">
        <p 
          className="leading-[22px] text-sm sm:text-base whitespace-pre-wrap"
          style={{ color: isDark ? '#b0b0b0' : '#717182' }}
        >
          ...
          <a
            className="underline decoration-solid underline-offset-2 cursor-pixel font-medium text-blue-600 hover:text-blue-700 dark:text-blue-400 dark:hover:text-blue-300 transition-all duration-300 hover:scale-105"
            href="https://auth.tally100.com/sign-in"
          >
            <span className="underline decoration-solid underline-offset-2 leading-[22px]">
              Tally100
            </span>
          </a>
          <span 
          ...
          >
            <span className="underline decoration-solid underline-offset-2 leading-[22px] text-sm sm:text-base">
              Kidshub
            </span>
          </a>
        </p>
...
function TallyLogo({ isDark }: { isDark: boolean }) {
  return (
    <div
...
function RenmoLogo({ isDark }: { isDark: boolean }) {
  return (
    <div
...
function KidshubLogo({ isDark }: { isDark: boolean }) {
  return (
    <div
...
function CraveLogoLogo({ isDark }: { isDark: boolean }) {
  return (
    <div
...
function CompanyLogos({ isDark }: { isDark: boolean }) {
  return (
    <div className="box-border content-stretch flex flex-wrap sm:flex-wrap md:flex-wrap lg:flex-nowrap xl:flex-nowrap 2xl:flex-nowrap gap-3 sm:gap-4 md:gap-6 lg:gap-8 xl:gap-12 2xl:gap-20 min-h-[35px] sm:min-h-[42px] md:min-h-[52px] lg:min-h-[74px] items-center justify-center p-0 relative shrink-0 w-full">
      <TallyLogo isDark={isDark} />
      <RenmoLogo isDark={isDark} />
      <KidshubLogo isDark={isDark} />
      <YuweenLogo isDark={isDark} />
      <CraveLogoLogo isDark={isDark} />
    </div>
  );
}

function HeroContainer({ isDark }: { isDark: boolean }) {
  return (
